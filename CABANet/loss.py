...
def logit_distill_loss(logits_t, logits_s, temperature):
...
def KLDivergence(feat_t, feat_s, temperature):
    assert feat_s.shape[-2:] == feat_t.shape[-2:]
    loss = F.kl_div(
        F.log_softmax(feat_s / temperature, dim=1),
        F.log_softmax(feat_t / temperature, dim=1),
        reduction='sum',
        log_target=True
    ) * (temperature * temperature) / feat_s.numel()
    return loss
...
class LinearBlock(nn.Module):
    def __init__(self, input_dim=2048, embed_dim=768, norm_act=True):
        super().__init__()
        self.proj = nn.Conv2d(input_dim, embed_dim, 1)
...
class AlignmentLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.tr_transforms = nn.ModuleList()
        self.cnn_transforms = nn.ModuleList()
        # self.tr_transforms.append(nn.Sequential(nn.Conv2d(64, 64, 3, padding=1, bias=False), nn.Conv2d(64, 64, 1)))
        # T1 T2 C1 C2
        self.tr_transforms.append(MLP(64, 64))
        self.tr_transforms.append(MLP(128, 128))
        self.cnn_transforms.append(nn.Conv2d(256, 64, 1))
        self.cnn_transforms.append(nn.Conv2d(512, 128, 1))
        # self.tr_transforms.append(nn.Conv2d(64, 64, 1))
        # self.tr_transforms.append(nn.Conv2d(128, 128, 1))

        # T3 T4 C3 C4
        self.tr_transforms.append(nn.Conv2d(320, 320, 1))
        self.tr_transforms.append(nn.Conv2d(512, 512, 1))
        self.cnn_transforms.append(MLP(1024, 320))
        self.cnn_transforms.append(MLP(2048, 512))
        # C1 C2
        # self.cnn_transforms.append(nn.Conv2d(256, 64, 1))
        # self.cnn_transforms.append(nn.Conv2d(512, 128, 1))
        # C3 C4
        # self.cnn_transforms.append(nn.Conv2d(1024, 320, 1))
        # self.cnn_transforms.append(nn.Conv2d(2048, 512, 1))

        # MLP
        # self.tr_transforms.append(MLP(64, 768))
        # self.tr_transforms.append(MLP(128, 768))
        # self.tr_transforms.append(MLP(320, 768))
        # self.tr_transforms.append(MLP(512, 768))


        # self.cnn_transforms.append(MLP(1024, 768))
        # self.cnn_transforms.append(MLP(2048, 768))

    def forward(self, align_feature):
        loss_inter = align_feature[0][0].new_tensor(0.0)
        for i in range(4):
            feat_t = align_feature[0][i]
            feat_s = align_feature[1][i]
            # if i < 2:
            feat_t = self.tr_transforms[i](feat_t)
            feat_s = self.cnn_transforms[i](feat_s)
            # else:
            #     feat_s = self.cnn_transforms[i](feat_s)
            ...
            loss_inter = loss_inter + KLDivergence(feat_t, feat_s, 4)
        return loss_inter
