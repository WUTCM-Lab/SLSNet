...
from loss import CrossEntropyLoss, Edge_loss, Edge_weak_loss, AlignmentLoss
...
class FullModel(nn.Module):
    ...
    def forward(self, input, label=None, train=True):
        ...
        if train and self.use_mixup and label is not None:
            ...
            return loss
        if self.align:
            output, align_feature = self.model(input)
        else:
        ...
        if train:
            ...
            if isinstance(output, (list, tuple)):
                if self.use_edge:
                    ...
                    loss_align = self.align_loss(align_feature)
                    losses += loss_align
...
def get_model(args2, device, models='DANet'):
    if models in ['swinT', 'resT', 'beit', 'cswin', 'volo', 'segformer']:
        print(models, args2.head)
    elif models in ['transformer', 'cctnet' , 'bs']:
        print(models, args2.trans_cnn, args2.head)
    else:
        print(models)
    # if args2.dataset in ['GE']:
    nclass = 3

    if models == 'bs':
        from models.bs import BSNet
        model = BSNet(transformer_name=args2.trans_cnn[0], cnn_name=args2.trans_cnn[1], nclass=nclass,
                      img_size=args2.crop_size[0], pretrained=True, aux=True, head=args2.head, edge_aux=args2.use_edge)
    ...
    if models == 'segformer':
        from models.segformer import mit_b2 as Segformer
        model = Segformer(nclass=nclass, img_size=args2.crop_size[0], pretrained=True, aux=True, head=args2.head, edge_aux=args2.use_edge)
    if models == 'swinT':
    ...
    if models == 'tcunet':
        ...
        model = TCUNet(img_size=512, depth=[3, 4, 6, 3], use_cnn=True, base_channel=3, embed_dim=16)
...
def train():
    ...
    if args2.dataset == "GE":
        dataset_train = GE(train=True, dataset=args2.dataset, crop_szie=args2.crop_size)
        dataset_val = GE(train=False, dataset=args2.dataset, crop_szie=args2.crop_size)
    elif args2.dataset == "HRSC":
        dataset_train = HRSC(train=True, dataset=args2.dataset, crop_szie=args2.crop_size)
        dataset_val = HRSC(train=False, dataset=args2.dataset, crop_szie=args2.crop_size)
    if distributed:
    ...
    for epoch in range(last_epoch, args2.end_epoch):
        ...
        model.train()
        setproctitle.setproctitle("TBD:" + str(epoch) + "/" + "{}".format(args2.end_epoch))
        ...
        if epoch > 0:  # test_epoch:
            miou, acc, f1, precision, recall, class_iou = validate(dataloader_val, device, model, args2)
            ...
            for i in range(3):
                class_iou[i] = (reduce_tensor(class_iou[i]).cpu() / world_size).item()

        if args2.local_rank == 0:
            print('miou:{}, acc:{}, f1:{}, precision:{}, recall:{}'.format(miou, acc, f1, precision, recall))
            print('sea:{}, land:{}, ship:{}'.format(class_iou[0], class_iou[1], class_iou[2]))

            if (epoch % 10 == 0 and epoch > 50) or (epoch > test_epoch):
            ...
            if miou >= best_miou and miou != 0:
                ...
                best_weight_name = weight_save_dir + '/{}_lr{}_epoch{}_batchsize{}_{}_best.pkl'.format(
                    args2.models, args2.lr, args2.end_epoch, args2.train_batchsize * world_size, args2.information)
...
def validate(dataloader_val, device, model, args2):
    ...
    for i in range(nclass):
        CLASS_IOU[i] = CLASS_IOU[i] + class_iou[i]
    ACC = ACC + acc
    ...
    CLASS_IOU = torch.tensor(CLASS_IOU).to(device)
    ...
    return MIOU, ACC, F1, Precision, Recall, CLASS_IOU
