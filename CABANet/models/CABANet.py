...
class _CAFM(nn.Module):
    ...
    def __init__(self, channels):
        ...
        # self.conv_value = MLP(channels, channels)
        # self.conv_query = MLP(channels, channels)
        # self.conv_key = MLP(channels, channels)
...
class CAFM(nn.Module):
    ...
    def __init__(self, channels_trans, channels_cnn, channels_fuse):
        ...
        # self.linear_trans = MLP(channels_trans, channels_fuse)
        # self.linear_cnn = MLP(channels_cnn, channels_fuse)
...
class PAFM(nn.Module):
    def __init__(self, channels_trans, channels_cnn, channels_fuse, residual=False):
        ...
        # channels_trans = [64, 128, 320, 512]
        # channels_fuse = [64, 128, 320, 512]
        # channels_cnn = [256, 512, 1024, 2048]
        self.channels_fuse = channels_fuse
        self.residual = residual
        self.conv_trans = nn.Conv2d(channels_trans, channels_fuse, kernel_size=1)
        self.conv_cnn = nn.Conv2d(channels_cnn, channels_fuse, kernel_size=1)
        self.conv_squeeze = nn.Conv2d(2, 2, 7, padding=3)
        self.conv_fuse = nn.Conv2d(channels_fuse, channels_fuse, kernel_size=1)
        if residual:
            self.conv = nn.Conv2d(channels_trans + channels_cnn, channels_fuse, kernel_size=1)
    ...
    def forward(self, x, y):
        ...
        attn = x * sig[:, 0, :, :].unsqueeze(1) + y * sig[:, 1, :, :].unsqueeze(1)
...
class BSNet(nn.Module):
    ...
    def __init__(self, transformer_name, cnn_name, nclass, img_size, aux=False, pretrained=False, head='seghead', edge_aux=False):
        ...
        self.model = mit_b2(nclass=3, img_size=512, aux=False, edge_aux=False, head='seghead', pretrained=True)
        self.transformer_backbone = self.model.backbone
        head_dim = [64, 128, 320, 512]  # self.model.embed_dims
        ...
        #  cnn_head_dim [256, 512, 1024, 2048]
        #  head_dim     [64, 128, 320, 512]
        self.fuse1 = PAFM(head_dim[0], self.cnn_head_dim[0], self.fuse_dim[0], residual=True)
        self.fuse2 = PAFM(head_dim[1], self.cnn_head_dim[1], self.fuse_dim[1], residual=True)
    ...
    def forward(self, x):
        ...
        align_feature = [out_backbone_trans, out_backbone_cnn]

        return outputs, align_feature
...
if __name__ == '__main__':
    ...
    model = BSNet(transformer_name='segformer', cnn_name='resnet50', nclass=6, img_size=512, aux=True,
                   edge_aux=False, head='fchpnhead', pretrained=False)
