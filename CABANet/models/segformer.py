...
class Segformer(nn.Module):
    def __init__(self, nclass, img_size, embed_dims, depths, num_heads, mlp_ratios, qkv_bias, norm_layer, sr_ratios, drop_rate, drop_path_rate,
        ...
        self.backbone = MixVisionTransformer(num_classes=nclass, img_size=img_size, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8],
        ...
        if pretrained_root is None:
            ...
            print("load pretrain segformer")
            self.backbone.load_state_dict(torch.load(pretrained_root), strict=False)
...
def mit_b2(nclass, img_size, pretrained=True, aux=False, head='seghead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/mit_b2.pth'
    else:
        pretrained_root = None
    model = Segformer(nclass=nclass, img_size=img_size, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8],
                      mlp_ratios=[4, 4, 4, 4],qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                      depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],drop_rate=0.0, drop_path_rate=0.1,head_dim=[64,128,320,512],
                      aux=aux, head=head, edge_aux=edge_aux, pretrained_root=pretrained_root)
    return model
...
if __name__ == '__main__':
    ...
    model_large = mit_b2(nclass=6, img_size=512, aux=False, edge_aux=False, head='seghead', pretrained=True)
